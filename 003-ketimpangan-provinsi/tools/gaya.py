...
def dasar(mode, judul, sub, sumber, n):
    ...
    fig.text(0.02, -0.02, f"Sumber: {sumber} | n = {idn(n, 0)}", fontsize=8 * fs, color=INK2)
    ax.grid(True, axis="x")
...
def simpan(fig, nama, mode, penuh=False):
    """Simpan PNG sesuai ukuran MODE. penuh=True: kanvas utuh tanpa potong rapat (untuk kartu)."""
    m = MODE[mode]
    m["out"].mkdir(parents=True, exist_ok=True)
    if penuh:
        fig.savefig(m["out"] / f"{nama}.png", dpi=m["dpi"], facecolor=KERTAS)
    else:
        fig.savefig(m["out"] / f"{nama}.png", dpi=m["dpi"], bbox_inches="tight", facecolor=KERTAS)
    plt.close(fig)
